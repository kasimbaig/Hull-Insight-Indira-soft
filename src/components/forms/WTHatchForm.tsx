...
interface HatchEntry {
  ...
  location: string;
  typeOfHatch: string;
  conditionOfHatchCoamingCover: string;
  conditionOfHatchCoaming: string;
  conditionOfDogClipsHinges: string;
  conditionOfWedges: string;
  surfaceTruenessOfMetallicCoaming: string;
  adhesionSizeOfRubberBeading: string;
...
const WTHatchForm: React.FC = () => {
  const [formData, setFormData] = useState<WTHatchData>({
    ins: '',
    totalRows: 1,
    hatchEntries: [
      {
        ...
        location: '',
        typeOfHatch: '',
        conditionOfHatchCoamingCover: '',
        conditionOfHatchCoaming: '',
        conditionOfDogClipsHinges: '',
        conditionOfWedges: '',
        surfaceTruenessOfMetallicCoaming: '',
        adhesionSizeOfRubberBeading: '',
  ...
  useEffect(() => {
    const savedDraft = localStorage.getItem('wt-hatch-draft');
    if (savedDraft) {
      try {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      localStorage.setItem('wt-hatch-draft', JSON.stringify(formData));
    }, 500);
    return () => clearTimeout(timeoutId);
  }, [formData]);

  const handleInputChange = (field: keyof WTHatchData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleHatchEntryChange = (id: number, field: keyof HatchEntry, value: string) => {
    setFormData(prev => ({
      ...prev,
      hatchEntries: prev.hatchEntries.map(entry =>
  ...
  const handleTotalRowsChange = (value: string) => {
    ...
    const currentRows = formData.hatchEntries.length;
    ...
    if (numRows > currentRows) {
      ...
      setFormData(prev => ({
        ...prev,
        totalRows: numRows,
        hatchEntries: [...prev.hatchEntries, ...newEntries]
      }));
      ...
      setFormData(prev => ({
        ...prev,
        totalRows: numRows,
        hatchEntries: prev.hatchEntries.slice(0, numRows)
      }));
    } else {
      setFormData(prev => ({
        ...prev,
  ...
  const validateForm = (): boolean => {
    ...
    // Validate all hatch entries
    formData.hatchEntries.forEach((entry, index) => {
      const requiredFields: (keyof HatchEntry)[] = [
        'location', 'typeOfHatch', 'conditionOfHatchCoamingCover', 'conditionOfHatchCoaming',
        'conditionOfDogClipsHinges', 'conditionOfWedges', 'surfaceTruenessOfMetallicCoaming',
        'adhesionSizeOfRubberBeading', 'materialOfRubberBeading', 'conditionOfRubberBeading',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.removeItem('wt-hatch-draft');
  ...
  const saveDraft = () => {
    if (!formData.hatchEntries[0]?.location.trim()) {
    ...
    const draftData: DraftData = {
      ...
      title: `WT Hatch - ${formData.hatchEntries[0]?.location || 'No Location'}`,
      data: { ...formData },
      timestamp: new Date().toISOString()
    };

    const existingDrafts = JSON.parse(localStorage.getItem('wt-hatch-drafts') || '[]');
    existingDrafts.push(draftData);
    localStorage.setItem('wt-hatch-drafts', JSON.stringify(existingDrafts));
  ...
  const deleteDraft = (draftId: string) => {
    ...
    localStorage.setItem('wt-hatch-drafts', JSON.stringify(updatedDrafts));
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-center">Type of Hatch</TableHead>
                        <TableHead className="text-center">Condition of Hatch Coaming Cover</TableHead>
                        <TableHead className="text-center">Condition of Hatch Coaming</TableHead>
                    ...
                    <TableBody>
                      {formData.hatchEntries.map((entry, index) => (
                        <TableRow key={entry.id}>
                          <TableCell className="text-center">{entry.id}</TableCell>
                          <TableCell>
                            <Input
                              value={entry.location}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'location', handleTextValidation(e.target.value))}
                              maxLength={50}
                          ...
                          <TableCell>
                            <Input
                              value={entry.typeOfHatch}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'typeOfHatch', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter type"
                            />
                            {errors[`typeOfHatch_${entry.id}`] && <p className="text-red-500 text-xs mt-1">{errors[`typeOfHatch_${entry.id}`]}</p>}
                          </TableCell>
                          <TableCell>
                            <Input
                              value={entry.conditionOfHatchCoamingCover}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'conditionOfHatchCoamingCover', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                            {errors[`conditionOfHatchCoamingCover_${entry.id}`] && <p className="text-red-500 text-xs mt-1">{errors[`conditionOfHatchCoamingCover_${entry.id}`]}</p>}
                          </TableCell>
                          <TableCell>
                            <Input
                              value={entry.conditionOfHatchCoaming}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'conditionOfHatchCoaming', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                            {errors[`conditionOfHatchCoaming_${entry.id}`] && <p className="text-red-500 text-xs mt-1">{errors[`conditionOfHatchCoaming_${entry.id}`]}</p>}
                          </TableCell>
                          <TableCell>
                            <Input
                              value={entry.conditionOfDogClipsHinges}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'conditionOfDogClipsHinges', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'conditionOfWedges', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'surfaceTruenessOfMetallicCoaming', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                          ...
                          <TableCell>
                            <Input
                              value={entry.adhesionSizeOfRubberBeading}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'adhesionSizeOfRubberBeading', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                            {errors[`adhesionSizeOfRubberBeading_${entry.id}`] && <p className="text-red-500 text-xs mt-1">{errors[`adhesionSizeOfRubberBeading_${entry.id}`]}</p>}
                          </TableCell>
                          <TableCell>
                            <Input
                              value={entry.materialOfRubberBeading}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'materialOfRubberBeading', handleTextValidation(e.target.value))}
                              maxLength={50}
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'conditionOfRubberBeading', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'conditionOfSpringPistonMechanism', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'conditionOfAutoLockRetainingArrangement', handleTextValidation(e.target.value))}
                              maxLength={50}
                              placeholder="Enter condition"
                            />
                          ...
                          <TableCell>
                            <Input
                              value={entry.itp}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'itp', handleTextValidation(e.target.value))}
                              maxLength={50}
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'preservationStatusOfDoors', handleTextValidation(e.target.value))}
                              maxLength={50}
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'lubrication', handleTextValidation(e.target.value))}
                              maxLength={50}
                          ...
                          <TableCell>
                            <Input
                              value={entry.uld}
                              onChange={(e) => handleHatchEntryChange(entry.id, 'uld', handleTextValidation(e.target.value))}
                              maxLength={50}
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => handleHatchEntryChange(entry.id, 'finalResult', handleTextValidation(e.target.value))}
                              maxLength={50}
                ...
                <TableBody>
                  {drafts.map((draft, index) => (
                    <TableRow key={draft.id}>
                      ...
                      <TableCell>{draft.data.hatchEntries[0]?.location || 'No Location Data'}</TableCell>
